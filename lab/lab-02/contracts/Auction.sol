...
contract Auction {
    ...
    enum Outcome {NOT_FINISHED, NOT_SUCCESSFUL, SUCCESSFUL}
    ...
    function settle() public {
        if (judgeAddress != address(0)) {
            require(
                ...
                "[ReqFail] Only the judge and the highest bidder can settle!"
            );
        }
        require(
            outcome == Outcome.SUCCESSFUL,
            "[ReqFail] Can't settle unless the auction finished successfully."
        );
    ...
    function refund() public {
        if (judgeAddress != address(0)) {
            require(
                ...
                "[ReqFail] Only the judge and the seller can refund!"
            );
        }
        require(
            highestBidderAddress != address(0),
            "[ReqFail] Can't refund to an undefined highest bidder."
        );
        require(
            outcome == Outcome.NOT_SUCCESSFUL,
            "Can't refund a non-failed auction."
        );
